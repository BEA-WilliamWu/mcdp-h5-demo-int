define([
    ...
    "jquery",
    "./model",
    "ojL10n!extensions/resources/nls/profile",
...
], function (ko, $, PartyModel, resourceBundle, HthApiPasswordUserContext, HthApiPasswordModel, HthApiPasswordResource) {
    "use strict";
    ...
    return function (rootParams) {
        ...
        self.hthApiPasswordNls = HthApiPasswordResource;
        self.showChangeHthApiPassword = ko.observable(false);

        let disposed = false;

        /** Profile entry visibility depends on credential state; Code is validated on submission. */
        if (HthApiPasswordUserContext.isHthUser(rootParams.dashboard.userData.userProfile)) {
            Promise.resolve().then(function () {
                return HthApiPasswordModel.status();
            }).then(function (data) {
                if (!disposed) {
                    self.showChangeHthApiPassword(Boolean(data &&
                        String(data.setupState || "").toUpperCase() === "ACTIVE"));
                }
            }).catch(function () {
                return null;
            });
        ...
        self.changeHthApiPassword = function () {
            if (self.showChangeHthApiPassword()) {
                rootParams.baseModel.registerComponent("api-password", "host-to-host");
                rootParams.dashboard.loadComponent("api-password", { mode: "RESET" });
            }
        };
        ...
        self.readPartyDetails = function () {
            if (rootParams.dashboard.appData.segment === "RETAIL") {
                Promise.all([
                    PartyModel.fetchCountry(),
                    PartyModel.fetchParty(),
                    PartyModel.fetchProfileConfig()
                    ...
                    for (i = 0; i < dataParty.party.addresses.length; i++) {
                        if (dataParty.party.addresses[i].type === "PST") {
                            self.fullAddress(dataParty.party.addresses[i].postalAddress);
                            self.address(self.serializer(dataParty.party.addresses[i].postalAddress));
                        }
                    }
        ...
        self.edit = function (data,value) {
            ...
            if(data==="EMAIL") {
                ...
                self.headerMessages()[0].summaryMessage=self.nls.emailUpdateMessage;
            }else{
                self.attributeValue(null);
                self.headerMessages()[0].summaryMessage=self.nls.successMessage;
           }

            $("#blockdetails_" + data).slideDown("slow");
            $("#blocktoshow_" + data).hide().trigger("closeModal");
        ...
        self.hideUpdatedDetails = function (data) {
            ...
            $("#blockdetails_" + data).hide().trigger("closeModal");
