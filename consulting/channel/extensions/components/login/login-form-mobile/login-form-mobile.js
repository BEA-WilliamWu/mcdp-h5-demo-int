    ...
    return function (rootParams) {
        ...
        function loginAuthenticator(genericViewModel) {
            ...
            Platform.getInstance("authentication").then(function (platform) {
                ...
                platform(loginType, username.toUpperCase(), self.password(), "customer", "015" + self.cdcId(), genericViewModel.queryMap).then(function () {
                        ...
                        if (error && error.type === "INVALID_CRED") {
                            ...
                            switch (window.decodeURIComponent(error.message)) {
                                case "INVALID_LOGIN_DETAILS":
                                    self.message(self.nls.loginForm.validationMsgs.INVALID_CRED);
                                    break;
                                case "MAX_NUMBER_OF_ATTEMPTS_REACHED":
                                    self.message(self.nls.loginForm.validationMsgs.MAX_NUMBER_OF_ATTEMPTS_REACHED);
                                    break;
