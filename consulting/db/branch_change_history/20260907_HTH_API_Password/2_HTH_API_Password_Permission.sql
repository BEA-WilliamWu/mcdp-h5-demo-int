...
DEFINE HTH_STATUS_ENT = com.ofss.digx.cz.bea.app.hosttohost.service.HostToHostApiPassword.status_View
DEFINE HTH_SETUP_ENT  = com.ofss.digx.cz.bea.app.hosttohost.service.HostToHostApiPassword.setup_Perform
DEFINE HTH_RESET_ENT  = com.ofss.digx.cz.bea.app.hosttohost.service.HostToHostApiPassword.reset_Perform
...
DELETE FROM DIGX_AZ_POLICY_ENT_MAP
 WHERE ENTITLEMENT_ID IN ('&HTH_STATUS_ENT', '&HTH_SETUP_ENT', '&HTH_RESET_ENT');
...
DELETE FROM DIGX_AZ_ENTITLEMENT
 WHERE ID IN ('&HTH_STATUS_ENT', '&HTH_SETUP_ENT', '&HTH_RESET_ENT');
...
INSERT ALL
  ...
  VALUES
    ('&HTH_STATUS_ENT', 'View HTH API Password Status',
  ...
  INTO DIGX_AZ_ENTITLEMENT
    (ID, NAME, DISPLAY_NAME, DESCRIPTION, CREATED_BY, CREATION_DATE,
     LAST_UPDATED_BY, LAST_UPDATE_DATE, ENTITY_STATUS, OBJECT_VERSION_NUMBER)
  VALUES
    ('&HTH_SETUP_ENT', 'Set Up HTH API Password',
  ...
  INTO DIGX_AZ_ENTITLEMENT
    (ID, NAME, DISPLAY_NAME, DESCRIPTION, CREATED_BY, CREATION_DATE,
     LAST_UPDATED_BY, LAST_UPDATE_DATE, ENTITY_STATUS, OBJECT_VERSION_NUMBER)
  VALUES
    ('&HTH_RESET_ENT', 'Reset HTH API Password',
...
INSERT ALL
  ...
  VALUES ('hthapipwdui-2026090701', 'VIW', 'hth-api-password', '&HTH_STATUS_ENT')
  INTO DIGX_AZ_RESOURCE_ACTION (ID, ACTION_TYPE, RESOURCE_ID, ENTITLEMENT_ID)
...
SELECT DISTINCT SOURCE_GROUP.ENT_GROUP_ID, TARGET_ENT.ENTITLEMENT_ID
  ...
  CROSS JOIN (
    SELECT '&HTH_STATUS_ENT' ENTITLEMENT_ID FROM DUAL
    UNION ALL SELECT '&HTH_SETUP_ENT' FROM DUAL
    UNION ALL SELECT '&HTH_RESET_ENT' FROM DUAL
  ) TARGET_ENT;
...
UNDEFINE HTH_STATUS_ENT
UNDEFINE HTH_SETUP_ENT
UNDEFINE HTH_RESET_ENT
