...
public class CZAsyncAuditHandler implements IAuditHandler {
	...
	private static void initializeHighRiskTxns() {
		if (highRiskTxnList == null) {
			...
			String[] highRiskTransactions = { "PC_F_GCRNDFT_FPS", "LAT_N_CA", "LAT_N_DA", "LAT_N_UA", "PC_N_CIP",
					...
					"MT_N_UUS", "FL_C_MIHO", "EB_F_BP", "EB_F_BP_SI", "BPFU_BP_FT"  };
	...
	public void doAudit(AuditDTO auditDTO) throws Exception {
        boolean hthAudit = HthOnboardingAudit.prepare(auditDTO);
        boolean hthRest = hthAudit && auditDTO.getAuditDetailsDTOList() != null
            && !auditDTO.getAuditDetailsDTOList().isEmpty()
            && auditDTO.getAuditDetailsDTOList().get(0).getAuditType() == Type.REST;
        try {
		AuditDetailsDTO auditDetailsDTO;

		List<String> exclusionTaskList = getAuditExclusionTaskTaskList();
		BeaSystemOut.println("inside Audit DTO for task code "+ auditDTO.getTaskCode() );
		...
		if(auditDTO!=null && auditDTO.getTaskCode()!=null && auditDTO.getTaskCode().equals("MT_N_CFM")) {
			BeaSystemOut.println("Inside do audit for Fps Merchant Addressing create");
		}
		...
		if (auditMap != null && auditMap.size() > 0) {
			...
			for (AuditMap listAuditMap : auditMap) {
                ...
                if(listAuditMap.getName().equalsIgnoreCase("x-device-model") || listAuditMap.getName().equalsIgnoreCase("x-device-os") || listAuditMap.getName().equalsIgnoreCase("x-device-os-version")){
                    BeaSystemOut.println("BCM Audit DTO for task code: " + auditDTO.getTaskCode() + ", status: " + auditDTO.getStatus());
                    if("BCM_ITK_OTP_VERIFY".equals(auditDTO.getTaskCode()) && com.ofss.digx.enumeration.audit.Status.SUCCESS == auditDTO.getStatus()){
                        return;
                    }
                    isBcmApp = true;
                    if("PC_CM_ME".equals(auditDTO.getTaskCode()) || "BCM_ITO_ME".equals(auditDTO.getTaskCode()) || "BCM_BIO_ME".equals(auditDTO.getTaskCode()) || "CH_I_AD".equals(auditDTO.getTaskCode()) || "CH_I_AS".equals(auditDTO.getTaskCode()) || "TD_I_AA".equals(auditDTO.getTaskCode())){
                        auditDTO.setAction(Action.ENQUIRED);
                    }
                    if("BCM_ITK_PA".equals(auditDTO.getTaskCode()) || "BCM_ITK_OTP_VERIFY".equals(auditDTO.getTaskCode()) || "BCM_ITK_EB".equals(auditDTO.getTaskCode()) || "BCM_ITK_DB".equals(auditDTO.getTaskCode()) || "BCM_ITK_CP".equals(auditDTO.getTaskCode()) || "BCM_ITK_RP".equals(auditDTO.getTaskCode())){
                        auditDTO.setAction(Action.INITIATED);
                    }
                    auditDTO.setAccessPointId("MOBILEAPP");
                    if (com.ofss.digx.infra.thread.ThreadAttribute.get(TRANSACTION_AUDIT_STATUS) != null
                        ...
                        com.ofss.digx.cz.bea.app.logger.BeaSystemOut.println("Failure status set in audit DTO, task code:" + auditDTO.getTaskCode() + ", status: " + com.ofss.digx.infra.thread.ThreadAttribute.get(TRANSACTION_AUDIT_STATUS));
		...
		if ( com.ofss.digx.infra.thread.ThreadAttribute.get("MULTIPLE_TXN_HIGH_RISK_INDICATOR") !=null) {
			...
			if (multiApproveValue.equalsIgnoreCase("Y")){
				BeaSystemOut.println("Multiapproval txn high risk indicator Y");
				indicator="Y";
			}
		...
		BeaSystemOut.println("IS_SUSPICIOUS_INDICATOR in asynch com.ofss.digx.infra.thread.ThreadAttribute"+ (String)com.ofss.digx.infra.thread.ThreadAttribute.get("IS_SUSPICIOUS_INDICATOR"));
		BeaSystemOut.println("IS_SUSPICIOUS_INDICATOR in asynch com.ofss.fc.infra.thread.ThreadAttribute"+ (String)com.ofss.fc.infra.thread.ThreadAttribute.get("IS_SUSPICIOUS_INDICATOR"));
		...
		if (com.ofss.digx.infra.thread.ThreadAttribute.get("IS_SUSPICIOUS_INDICATOR") !=null) {
			...
			BeaSystemOut.println("isSuspiciousIndicatorFromDict "+isSuspiciousIndicatorFromDict);
			if (isSuspiciousIndicatorFromDict.equalsIgnoreCase("Y") || isSuspiciousIndicatorFromDict.equalsIgnoreCase("SUS") || isSuspiciousIndicatorFromDict.equalsIgnoreCase("true")){
				BeaSystemOut.println("IS_SUSPICIOUS_INDICATOR in asynch audit true");
		...
		if (com.ofss.fc.infra.thread.ThreadAttribute.get("IS_SUSPICIOUS_INDICATOR") !=null) {
			...
			BeaSystemOut.println("isSuspiciousIndicatorFromDict com.ofss.fc.infra.thread.ThreadAttribute "+isSuspiciousIndicatorFromDict);
			if (isSuspiciousIndicatorFromDict.equalsIgnoreCase("Y") || isSuspiciousIndicatorFromDict.equalsIgnoreCase("SUS") || isSuspiciousIndicatorFromDict.equalsIgnoreCase("true")){
				BeaSystemOut.println("IS_SUSPICIOUS_INDICATOR in asynch audit true com.ofss.fc.infra.thread.ThreadAttribute");
				suspMap.setValue("yes");
		...
		auditMap.add(suspMap);
		  ...
		  map.setName("IPRange");
			if (com.ofss.digx.infra.thread.ThreadAttribute.get("FMO_IP_ADDRESS")!=null && !("".equalsIgnoreCase((String) com.ofss.digx.infra.thread.ThreadAttribute.get("FMO_IP_ADDRESS"))) ){
				...
				BeaSystemOut.println("The FMO IP address is "+iPAddress);
            ...
            if(isBcmApp){
                if(auditDTO.getUserId()==null || "anonymous".equals(auditDTO.getUserId())){
                    auditDTO.setUserId((String) com.ofss.digx.infra.thread.ThreadAttribute.get("SUBJECT_NAME"));
                }
                if(auditDTO.getUserType()==null){
                    auditDTO.setUserType("corporateuser");
                }
                com.ofss.digx.cz.bea.app.logger.BeaSystemOut.println("auditDTO.userId value is " + auditDTO.getUserId() + ", userType value is " + auditDTO.getUserType() + ", action value is " + auditDTO.getAction()
                        + ", status value is " + auditDTO.getStatus()+ ", accessPointId value is " + auditDTO.getAccessPointId());
            }
			...
			try {
				BeaSystemOut.println("CZAsynchAudit read User session");
				if (DataAccessManager.getManager().isSessionOpen()) {
					BeaSystemOut.println("CZAsynchAudit session is open");
					session = DataAccessManager.getManager().fetchCurrentSession();
				} else {
					BeaSystemOut.println("CZAsynchAudit Opening DIGX session");
					session = DataAccessManager.getManager().openSession("DIGX");
					isSessionOpen = true;
				}

				if(session != null) {
					BeaSystemOut.println("CZAsynchAudit session not null");
					...
					BeaSystemOut.println("Login Time set "+loginTime.toString("yyyy-MM-dd'T'HH:mm:ss.SSSz"));

				}else {
					BeaSystemOut.println("CZAsynchAudit session is null");
					map.setValue("");
				}

			} catch (java.lang.Exception e) {
				BeaSystemOut.println("Error while reading session for CRM");
				BeaSystemOut.printErr(e);
				map.setValue("");
			}finally {
				if (isSessionOpen) {
					DataAccessManager.getManager().closeSession(session);
					BeaSystemOut.println("Session closed");
				}
			}
			}else {
				map.setValue("");
		...
		case REST:
			...
			if (!hthAudit && logger.isLoggable(Level.FINE)) {
			...
			if (restRequestAuditEnabled) {
			  ...
			  if (null != auditDTO.getHttpMethod())
              {
                              ...
                              {
                            	  BeaSystemOut.println("Inside the audit API response data");
			...
			if (auditDTO.getUserId() != null & !ANONYMOUS_USER.equals((auditDTO.getUserId()))) {
				...
				if(userProfileDomain!=null ) {
					...
					BeaSystemOut.println("AuditDTO username set from CZ_UM_USERPROFILE = " + usernameFromUMuserprofile);
...
//				}
				...
				BeaSystemOut.println("CZAsyncAuditHandler userPartyDetailsRequestDTO.getPartyId() "+userPartyDetailsRequestDTO.getPartyId());
				...
				if (partyDetails != null && partyDetails.getCompanyName() != null) {
					BeaSystemOut.println("CZAsyncAuditHandler partyDetails.getCompanyName() "+partyDetails.getCompanyName());
			...
			if (auditDTO.getReferenceNo() == null || auditDTO.getReferenceNo() == "") {
				...
				if (com.ofss.fc.infra.thread.ThreadAttribute
					...
					BeaSystemOut.println("CZAsync Ref No FC="+referenceNumber);
					...
					if (referenceNumber == null) {
						...
						BeaSystemOut.println("CZAsync Ref No DIGX="+referenceNumber);
					}
					...
					BeaSystemOut.println("Ref No for QR Code Generation");
			...
			if(com.ofss.digx.infra.thread.ThreadAttribute.get(com.ofss.digx.infra.thread.ThreadAttribute.FILE_REF_ID) != null) {
				...
				BeaSystemOut.println("** Audit file ref no="+fileRefNo);
			...
			if (com.ofss.digx.infra.thread.ThreadAttribute.get(TRANSACTION_AUDIT_STATUS) != null
					&& com.ofss.digx.infra.thread.ThreadAttribute.get(TRANSACTION_AUDIT_STATUS)
							.equals(ProcessingStatus.FAIL)) {
				BeaSystemOut.println("Failure status set in audit DTO");
				auditDTO.setStatus(com.ofss.digx.enumeration.audit.Status.FAILURE);
			}

			if (auditDTO.getReferenceNo() != null && auditDTO.getTaskCode() != null
					&& auditDTO.getTaskCode().equals("FU_F_APC")) {
				BeaSystemOut.println("** Inside audit FU_F_APC");
				...
				if (auditDTO.getTaskCode() != null && auditDTO.getTaskCode().equals("TL_N")) {
					BeaSystemOut.println("** Inside audit TL_N");
					if (auditDTO.getAction() != null && auditDTO.getAction().equals(Action.CREATED)) {
						auditDTO.setAction(Action.ENQUIRED);
					}
				}
			...
			if (getOMBAuditFlag()) {
				...
				if (com.ofss.fc.infra.thread.ThreadAttribute
					...
					referenceNumber = ((String) com.ofss.fc.infra.thread.ThreadAttribute
							.get(com.ofss.fc.infra.thread.ThreadAttribute.TRANSACTION_REFERENCE_NO));
					BeaSystemOut.println("CZAsync Ref No FC audit=" + referenceNumber);

					if (referenceNumber == null) {
						referenceNumber = (String) com.ofss.digx.infra.thread.ThreadAttribute
								.get(com.ofss.digx.infra.thread.ThreadAttribute.TRANSACTION_REFERENCE_NO);
						BeaSystemOut.println("CZAsync Ref No DIGX audit=" + referenceNumber);
					}
				...
				if (referenceNumber == null && com.ofss.digx.infra.thread.ThreadAttribute.get("AUDIT_TXN_REF_NO") != null) {
					...
					BeaSystemOut.println("Audit txn ref no=" + referenceNumber);
				}
				...
				if (referenceNumber != null) {
					...
					BeaSystemOut.println("Reading transaction domain for audit log=" + transaction.getKey().getId());
				...
				BeaSystemOut.println("CZAsyncAudit Handler User Id : " + auditDTO.getUserId() + " Task Code : "
				...
				if (auditDTO.getTaskCode() != null && auditDTO.getTaskCode().equals("IVM_R_A")) {
					BeaSystemOut.println("** Inside audit IVM_R_A , status: " + auditDTO.getStatus());
					if(com.ofss.digx.enumeration.audit.Status.FAILURE.equals(auditDTO.getStatus())) {
						...
						if(auditDTO.getResolvedRequestUrl()!=null && auditDTO.getResolvedRequestUrl().contains("getRaq")) {
							BeaSystemOut.println("IVM_R_A audit log getRaq");
							auditDTO.setAction(Action.INITIATED);
						} else if(auditDTO.getResolvedRequestUrl()!=null && auditDTO.getResolvedRequestUrl().contains("create")) {
							BeaSystemOut.println("IVM_R_A audit log create");
							auditDTO.setAction(Action.CREATED);
						} else {
							BeaSystemOut.println("IVM_R_A audit log ENQUIRED");
				...
				if(auditDTO.getTaskCode() != null && auditDTO.getTaskCode().equals("MT_C_EOSS")) {
					boolean viewFlag = false;
					List<AuditMap> queryParamList=auditDTO.getQueryParamList();
					for (AuditMap queryParam : queryParamList) {
						BeaSystemOut.println("MT_C_EOSS::queryParam.getName="+queryParam.getName()+",queryParam.getValue="+queryParam.getValue());
						String queryParamValue0="";
						String[] queryParamValueArr = queryParam.getValue().split(",");
						if(queryParamValueArr.length>0) {
							queryParamValue0 = queryParamValueArr[0];
						}
						if(queryParam.getName().equalsIgnoreCase("fromMainPage") && queryParamValue0.equalsIgnoreCase("Y")) {
							BeaSystemOut.println("MT_C_EOSS from mainPage!");
							if (auditDTO.getAction() != null) {
								auditDTO.setAction(Action.ENQUIRED);
								viewFlag = true;
								break;
							}
						}else if(queryParam.getName().equalsIgnoreCase("fromMainPage") && queryParamValue0.equalsIgnoreCase("N")) {
							BeaSystemOut.println("MT_C_EOSS NOT from mainPage!");
							break;
						}
					}
					if(!viewFlag) {
						BeaSystemOut.println("MT_C_EOSS will not show view!");
						break;
					}
				}
				
				if(auditDTO.getTaskCode() != null && auditDTO.getTaskCode().equals("MT_C_MOSS")) {
					Action auditAction = auditDTO.getAction();
					com.ofss.digx.enumeration.audit.Status auditStatus = auditDTO.getStatus();	
					BeaSystemOut.println("MT_C_MOSS auditAction="+auditAction);
					BeaSystemOut.println("MT_C_MOSS auditStatus="+auditStatus);
					
					if(auditAction != null && auditAction.equals(Action.EDITED)) {
						if(com.ofss.digx.enumeration.audit.Status.FAILURE.equals(auditStatus)) {
								BeaSystemOut.println("MT_C_MOSS input Failed, skiped");
								break;
						}
					}
				}

			if(auditDTO!=null && auditDTO.getTaskCode()!=null && (auditDTO.getTaskCode().equalsIgnoreCase("MT_N_RGNBD") || auditDTO.getTaskCode().equalsIgnoreCase("MT_N_RGNB"))){
				auditDTO.setAction(Action.ENQUIRED);
			}
				...
				if (!ANONYMOUS_USER.equals(auditDTO.getUserId()) || !auditDTO.getTaskCode().equalsIgnoreCase("PC_CM_ME")
					...
					if(!(auditDTO.getResolvedRequestUrl()!=null && auditDTO.getResolvedRequestUrl().contains("requestModification"))) {
						...
						if(auditDTO!=null && auditDTO.getTaskCode()!=null && auditDTO.getTaskCode().equals("MT_N_CFM")) {
							BeaSystemOut.println("Inside getAsyncHandler().doAudit for Fps Merchant Addressing create");
						}
						if (auditDTO!=null && auditDTO.getTaskCode()!=null && auditDTO.getTaskCode().equals("EADESTMTC")) {
							BeaSystemOut.println("Inside do audit for Account Preference Setting create");
							if(validationOnly)
								break;
						}
						...
						if(auditDTO!=null && auditDTO.getTaskCode()!=null && auditDTO.getTaskCode().equals("PC_F_BTFT")) {
							...
							auditDTO.setAccessKey("");
							auditDTO.setVerified(com.ofss.digx.enumeration.audit.Status.SUCCESS);
                            BeaSystemOut.println("audit list size"+auditDTO.getAuditDetailsDTOList().size());
                            BeaSystemOut.println("insert audit log "+ SerializationUtils.toJsonString(auditDTO.getAuditDetailsDTOList()));
							List<AuditDetailsDTO> collect = auditDTO.getAuditDetailsDTOList().stream().filter(e -> !(e.getRequest() instanceof BatchTransferRequestDTO)).collect(Collectors.toList());
							auditDTO.setAuditDetailsDTOList(collect);
						}

						if(auditDTO!=null && auditDTO.getTaskCode()!=null && auditDTO.getTaskCode().equals("BPFU_BP_FT")) {
							if(auditDTO.getResolvedRequestUrl().contains("confirm")){
								auditDTO.setId("");
								auditDTO.setPartyName("");
								auditDTO.setHashValue("");
								auditDTO.setAccessKey("");
								auditDTO.setVerified(com.ofss.digx.enumeration.audit.Status.SUCCESS);
								List<AuditDetailsDTO> collect = auditDTO.getAuditDetailsDTOList().stream().filter(e -> !(e.getRequest() instanceof BulkPaymentRequestDTO)).collect(Collectors.toList());
								auditDTO.setAuditDetailsDTOList(collect);
							}else if(auditDTO.getResolvedRequestUrl().contains("files")){
								auditDTO.setReferenceNo(" ");
							}
						}
                        if (hthAudit) HthOnboardingAudit.finish(auditDTO);
                        BeaSystemOut.println("insert audit log "+ SerializationUtils.toJsonString(auditDTO));
						getAsyncHandler().doAudit(auditDTO);
					}
				}
            BeaSystemOut.println("Sent Audit DTO for task code "+ auditDTO.getTaskCode() );
			break;
		case ALL:
			break;
		case ENDPOINT:
            ...
            if (hthRest) HthOnboardingAudit.clearRequestStack();
        }
	}
	...
	public void doApiAudit(APIAuditDTO audit) throws Exception {
		if (ThreadAttribute.get(ThreadAttribute.SUBJECTNAME) != null) {
			audit.setUserId(ThreadAttribute.get(ThreadAttribute.SUBJECTNAME).toString());
		}
		if (ThreadAttribute.get(ThreadAttribute.ENTERPRISE_ROLE_ID) != null) {
			audit.setUserSegment(ThreadAttribute.get(ThreadAttribute.ENTERPRISE_ROLE_ID).toString());
		}
	}
	...
	private String getIPRangeValue(String ipAddress) {
		...
		BeaSystemOut.println("The IP range value is "+ipRangeValue);
