...
public class HostToHostApiPassword extends AbstractApplication
  ...
  public HthApiPasswordCodeResponseDTO reveal(SessionContext sessionContext,
    ...
    try (HthOnboardingAudit.Entry audit = HthOnboardingAudit.begin(sessionContext, REVEAL_SERVICE_ID, "REVEAL")) {
    try {
    ...
    try {
      ...
      audit.put("partyId", row.getPartyId())
          .put("targetUserId", HthOnboardingAudit.fullUser(row.getUserName(), row.getPartyId()))
          .put("codeId", row.getKey().getId()).put("purpose", row.getPurpose()).put("codeStatus", row.getStatus());
    ...
    super.checkResponsePolicy(sessionContext, response);
    audit.result("SUCCESS").response(response);
    return response;
    } catch (java.lang.Exception failure) {
      audit.failure(failure);
      throw failure;
