...
import com.ofss.digx.cz.bea.app.hosttohost.dto.EligibleAccountDTO;
...
public class HostToHostManagement extends AbstractApplication implements IHostToHostManagement {
    ...
    private static final String HTH_API_SERVICE_SUBMIT_SUCCESS_EVENT = "HTH_API_SERVICE_SUBMIT_SUCCESS";
    private static final String HTH_API_SERVICE_EDIT_SUCCESS_EVENT = "HTH_API_SERVICE_EDIT_SUCCESS";
    private static final String HTH_API_SERVICE_DISABLE_SUCCESS_EVENT = "HTH_API_SERVICE_DISABLE_SUCCESS";

    private static final String ACTIVITY_HTH_API_SERVICE_SUBMIT =
            "com.ofss.digx.cz.bea.app.hosttohost.service.HostToHostManagement.submit";
    private static final String ACTIVITY_HTH_API_SERVICE_EDIT =
            "com.ofss.digx.cz.bea.app.hosttohost.service.HostToHostManagement.edit";
    private static final String ACTIVITY_HTH_API_SERVICE_DISABLE =
    ...
    private static final String GET_ELIGIBLE_ACCOUNTS_SERVICE_ID =
            "com.ofss.digx.cz.bea.app.hosttohost.service.HostToHostManagement.getEligibleChargeAccounts";
    ...
    private static final Set<String> ELIGIBLE_PRODUCT_CODES = new LinkedHashSet<String>(Arrays.asList(
            "6805", "6802", "6803", "4040", "4060", "4050", "1010", "1014"
    ));
    ...
    @Entitlement(name = "Search Host To Host Management", action = ActionType.VIEW, requiredResources = {})
    @EntitlementGroup(category = EntitlementCategory.ADMIN_MAINTENANCE, subCategory = EntitlementSubCategory.Party_Preference)
    public List<EligibleAccountDTO> getEligibleChargeAccounts(SessionContext sessionContext,
                                                               String partyId,
                                                               String accountNumber) throws Exception {
        if (logger.isLoggable(Level.FINE)) {
            logger.log(Level.FINE, formatter.formatMessage(
                    "Entered into getEligibleChargeAccounts() : partyId = %s, accountNumber = %s in class %s",
                    partyId, accountNumber, THIS_COMPONENT_NAME));
        }

        List<EligibleAccountDTO> eligibleAccounts = new ArrayList<EligibleAccountDTO>();

        String queryPartyId = !isBlank(accountNumber) ? accountNumber : partyId;
        if (isBlank(queryPartyId)) {
            return eligibleAccounts;
        }

        try {
            eligibleAccounts = fetchEligibleChargeAccountsForParty(queryPartyId, sessionContext);
        } catch (Exception e) {
            logger.log(Level.SEVERE, formatter.formatMessage(
                    "Exception from getEligibleChargeAccounts() for partyId '%s' in class %s",
                    partyId, THIS_COMPONENT_NAME), e);
            throw e;
        } catch (RuntimeException rte) {
            logger.log(Level.SEVERE, formatter.formatMessage(
                    "RuntimeException from getEligibleChargeAccounts() for partyId '%s' in class %s",
                    partyId, THIS_COMPONENT_NAME), rte);
            throw new Exception(rte);
        }

        if (logger.isLoggable(Level.FINE)) {
            logger.log(Level.FINE, formatter.formatMessage(
                    "Exiting from getEligibleChargeAccounts() : count = %s",
                    String.valueOf(eligibleAccounts.size())));
        }
        return eligibleAccounts;
    }
    ...
    private String getActivityId(String actionType) {
        ...
        if (ACTION_ENABLE.equals(actionType)) {
            return ACTIVITY_HTH_API_SERVICE_SUBMIT;
        }

        if (ACTION_EDIT.equals(actionType)) {
            return ACTIVITY_HTH_API_SERVICE_EDIT;
        }

        if (ACTION_DISABLE.equals(actionType)) {
            return ACTIVITY_HTH_API_SERVICE_DISABLE;
        }

        return ACTIVITY_HTH_API_SERVICE_EDIT;
    }

    private String getEventId(String actionType) {

        if (ACTION_ENABLE.equals(actionType)) {
            return HTH_API_SERVICE_SUBMIT_SUCCESS_EVENT;
        }

        if (ACTION_EDIT.equals(actionType)) {
            return HTH_API_SERVICE_EDIT_SUCCESS_EVENT;
        }

        if (ACTION_DISABLE.equals(actionType)) {
            return HTH_API_SERVICE_DISABLE_SUCCESS_EVENT;
        }
    ...
    private boolean shouldNotifyCompanyChange(SessionContext context, HostToHostManagementDTO request,
        ...
        try {
            ...
            if (current.getKey() == null) {
                logCompanyNotification(request, actionType, "SNAPSHOT", "NONE", "MissingManagementKey");
                return false;
            }
            return !fetchSelectedApiCodes(current.getKey().getId()).equals(extractSelectedApiCodes(request));
        } catch (java.lang.Exception e) {
            logCompanyNotification(request, actionType, "SNAPSHOT", "NONE", e.getClass().getSimpleName());
            return false;
        }
    }

    private void notifyCompanyChange(SessionContext context, HostToHostManagementDTO request, String actionType) {
        String stage = "CONTACT";
        String channel = "NONE";
        try {
            ...
            CZPartyPreferenceDTO company = adapter.getPartyPreferences(request.getPartyId());
            if (company == null) {
                logCompanyNotification(request, actionType, stage, channel, "MissingCompanyContact");
                return;
            }
            NotificationDetail detail = new NotificationDetail();
            String email = normalize(company.getOfficeEmailId());
            if (email != null) {
                channel = "EMAIL";
                // An invalid but populated email is not permission to fall back to SMS.
                javax.mail.internet.InternetAddress address = new javax.mail.internet.InternetAddress(email, true);
                address.validate();
                if (!email.equals(address.getAddress())) {
                    logCompanyNotification(request, actionType, stage, channel, "InvalidCompanyEmail");
                    return;
                }
                detail.setDestination(DestinationType.EMAIL);
                detail.setDispatchAddress(email);
            } else {
                channel = "SMS";
                String sms = companySmsAddress(company.getOfficeTelNo());
                if (sms == null) {
                    logCompanyNotification(request, actionType, stage, channel, "MissingOrAmbiguousCompanyMobile");
                    return;
                }
                // Fail closed if deployment missed the HTH-only skip-list addition: never borrow BM's country.
                String skip = com.ofss.digx.cz.bea.app.customconfig.util.CustomConfigUtil.readConfigValue(
                        "SMS_DISPATCHER_SKIP_COUNTRY_CODE_EVENTID_LIST", "");
                if (skip == null || !Arrays.asList(skip.toUpperCase(Locale.ROOT).replace(" ", "").split(","))
                        .contains(getEventId(actionType))) {
                    logCompanyNotification(request, actionType, "CONFIG", channel, "MissingSmsCountryRouting");
                    return;
                }
                detail.setDestination(DestinationType.SMS);
                detail.setDispatchAddress(sms);
            }
            // The company adapter does not populate partyIdValue; use the validated request party.
            detail.setRecipientType(SubscriberType.EXTERNAL.toString());
            detail.setRecipientId(request.getPartyId());
            UserProfUpdateActivityLogDTO log = new UserProfUpdateActivityLogDTO();
            log.setCustomerId(request.getPartyId());
            log.setNotificationDetails(new NotificationDetail[]{detail});
            stage = "REGISTER";
            TransactionStatus status = super.registerActivityAndGenerateEvent(
                    context, getActivityId(actionType), getEventId(actionType), new Date(), log);
            logCompanyNotification(request, actionType, stage, channel,
                    status != null && status.getReplyCode() == 0 ? "Registered" : "UnsuccessfulStatus");
        } catch (java.lang.Exception e) {
            // Do not log addresses, message content or exception messages. An already rollback-only
            // transaction is still handled by the existing Interaction.close() boundary.
            logCompanyNotification(request, actionType, stage, channel, e.getClass().getSimpleName());
        }
    }

    private String companySmsAddress(String officeMobile) {
        String value = normalize(officeMobile);
        if (value == null) {
            return null;
        }
        // Company preferences already map officeTelNo to WORK_MOBILE. The maintenance UI
        // accepts +country-number / +country number. A bare number has no authoritative country.
        java.util.regex.Matcher phone = java.util.regex.Pattern.compile(
                "^\\+([1-9][0-9]{0,2})[- ]([0-9]{8,14})$").matcher(value);
        if (!phone.matches() || phone.group(1).length() + phone.group(2).length() > 15) {
            return null;
        }
        return phone.group(2) + "~" + phone.group(1);
    }

    private void logCompanyNotification(HostToHostManagementDTO request, String actionType,
                                        String stage, String channel, String outcome) {
        String reference = request == null ? null : normalize(request.getReferenceNumber());
        if (reference == null || !reference.matches("[A-Za-z0-9._:-]{1,80}")) {
            reference = "UNAVAILABLE";
        }
        logger.log("Registered".equals(outcome) ? Level.INFO : Level.WARNING, formatter.formatMessage(
                "[HTH-1288] reference=%s, action=%s, stage=%s, channel=%s, outcome=%s",
                reference, actionType, stage, channel, outcome));
    }
    ...
    private void fillEffectiveManagement(HthManagement management, HostToHostManagementDTO requestDTO,
        ...
        management.setChargeAccount(requestDTO.getChargeAccount());
    ...
    private HthRequest buildRequest(HostToHostManagementDTO requestDTO, String actionType, String referenceNumber,
        ...
        request.setChargeAccount(requestDTO.getChargeAccount());
    ...
    private void assertConfigurationChangedForEdit(HostToHostManagementDTO requestDTO) throws Exception {
        ...
        if (isApprovalFlowUnchanged(currentManagement, requestDTO)
                && currentApiCodes.equals(requestedApiCodes)
                && isChargeAccountUnchanged(currentManagement, requestDTO)) {
            throw new IllegalArgumentException(
    ...
    private boolean isChargeAccountUnchanged(HthManagement currentManagement, HostToHostManagementDTO requestDTO) {
        String currentChargeAccount = normalize(currentManagement.getChargeAccount());
        String requestedChargeAccount = normalize(requestDTO.getChargeAccount());
        return currentChargeAccount == null
                ? requestedChargeAccount == null
                : currentChargeAccount.equals(requestedChargeAccount);
    }
    ...
    private List<EligibleAccountDTO> fetchEligibleChargeAccountsForParty(String partyId,
                                                                          SessionContext sessionContext) throws Exception {
        List<EligibleAccountDTO> result = new ArrayList<EligibleAccountDTO>();
        ...
        try {
            ...
            try {
                ...
                Query query = session.createSQLQuery(
                        ...
                        + "P.PRODUCT_DESCRIPTION_SHORT_TC, "
                        + "P.PRODUCT_DESCRIPTION_SHORT_SC "
                        + "FROM DIGX_PI_PARTY_ACCOUNTS A "
                        + "LEFT JOIN DIGX_CZ_PRODUCT_DETAILS P ON P.PRODUCT_CODE = "
                        + "NVL(A.PRODUCT_CODE, CASE WHEN "
                        + "REGEXP_SUBSTR(A.ACCOUNT_NUMBER, '[^~]+', 1, 3) IS NOT NULL THEN "
                        + "REGEXP_SUBSTR(A.ACCOUNT_NUMBER, '[^~]+', 1, 2) END) "
                ...
                java.util.List rows = query.list();
                logger.log(Level.INFO, formatter.formatMessage(
                        "[CHARGE-ACCT-DIAG] partyId=[%s], totalRows=%s, eligibleCodes=%s",
                        partyId,
                ...
                if (rows != null) {
                    for (Object rowValue : rows) {
                        ...
                        String productCode = String.valueOf(row[2]).trim();
                        if (!ELIGIBLE_PRODUCT_CODES.contains(productCode)) {
                            continue;
                        }
                        String accountNumber = String.valueOf(row[0]).trim();
                        String accountName = row[1] != null ? String.valueOf(row[1]).trim() : "";
                        EligibleAccountDTO dto = new EligibleAccountDTO();
                        dto.setAccountNumber(accountNumber);
                        dto.setAccountName(accountName);
                        dto.setProductCode(productCode);
                        dto.setProductDescription(resolveProductDescription(row));
                        result.add(dto);
                    }
                }
            } finally {
                if (session != null && openedSession) {
                    DataAccessManager.getManager().closeSession(session);
                }
            }
        } catch (java.lang.Exception e) {
            logger.log(Level.WARNING, formatter.formatMessage(
                    "Failed to fetch party accounts for charge account selection, partyId='%s' in class %s",
                    partyId, THIS_COMPONENT_NAME), e);
        }
        return result;
    }

    private String resolveProductDescription(Object[] row) {
        String en = row.length > 3 && row[3] != null ? String.valueOf(row[3]).trim() : null;
        String tc = row.length > 4 && row[4] != null ? String.valueOf(row[4]).trim() : null;
        String sc = row.length > 5 && row[5] != null ? String.valueOf(row[5]).trim() : null;
        String locale = CZLocaleUtils.getUserLocale();
        if ("zh-Hant".equalsIgnoreCase(locale) && tc != null) {
            return tc;
        }
        if ("zh-Hans-CN".equalsIgnoreCase(locale) && sc != null) {
            return sc;
        }
        return en != null ? en : tc;
    }
    ...
    private void populateResponse(HostToHostManagementResponseDTO response, HostToHostManagementDTO detail) {
        ...
        response.setChargeAccount(detail.getChargeAccount());
    ...
    private void populateStoredHostToHostConfig(HostToHostManagementDTO record, String partyId) throws Exception {
        ...
        if (management != null) {
            ...
            if (HTH_STATUS_ENABLE.equals(hthStatus)) {
                ...
                record.setChargeAccount(management.getChargeAccount());
            }
        }
