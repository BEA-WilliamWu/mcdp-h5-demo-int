...
public interface IHthApiPasswordOperationRepositoryAdapter extends IRepositoryAdapter<HthApiPasswordOperation, HthApiPasswordOperationKey> {
  ...
  /** Records the business client bound to a DSP reservation in the same transaction. */
  int bindDspClient(Session session, String requestId, String clientId) throws Exception;

  /** Reads confirmed or unresolved DSP writes for the current business client only. */
  List findDspState(Session session, String partyId, String userId, String clientId) throws Exception;
