...
public class LocalHthApiPasswordOperationRepositoryAdapter extends AbstractLocalRepositoryAdapter<HthApiPasswordOperation>
  ...
  public List findResult(Session session, String requestId, String partyId, String userId, String operation) throws Exception {
    Query query = session.createSQLQuery(
        "SELECT STATUS, REFERENCE_NUMBER, STORAGE_BACKEND, REMOTE_CLIENT_ID FROM HTH_BEA.HTH_API_PASSWORD_OPERATION "
  ...
  @Override
  public int bindDspClient(Session session, String requestId, String clientId) throws Exception {
    Query query = session.createSQLQuery(
        "UPDATE HTH_BEA.HTH_API_PASSWORD_OPERATION SET REMOTE_CLIENT_ID = ? "
            + "WHERE REQUEST_ID = ? AND STORAGE_BACKEND = 'DSP' AND STATUS = 'IN_PROGRESS' "
            + "AND REMOTE_CLIENT_ID IS NULL");
  ...
  @Override
  public List findDspState(Session session, String partyId, String userId, String clientId) throws Exception {
    Query query = session.createSQLQuery(
        "SELECT STATUS FROM HTH_BEA.HTH_API_PASSWORD_OPERATION "
            + "WHERE PARTY_ID = ? AND USER_ID = ? AND STORAGE_BACKEND = 'DSP' "
            + "AND REMOTE_CLIENT_ID = ? AND STATUS IN ('SUCCESS', 'IN_PROGRESS', 'UNKNOWN') "
            + "ORDER BY CASE WHEN STATUS = 'SUCCESS' THEN 1 ELSE 0 END, LAST_UPDATE_DATE DESC");
