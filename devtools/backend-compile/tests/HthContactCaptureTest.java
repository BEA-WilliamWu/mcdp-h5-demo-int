...
public final class HthContactCaptureTest {
    public static void run() throws Exception {
        ...
        Bank.check("BOTH".equals(snapshot.change) && snapshot.recipients.size()==6,"both changes: 4 API plus 2 final approver");
        ...
        Bank.check(HthProfileContactNotification.capture(context,request,old,ext).recipients.size()==5,"email-only retains one unchanged mobile");
        next.setEmailId(old.getEmailId());next.setMobileNumber("69999999");
        Bank.check(HthProfileContactNotification.capture(context,request,old,ext).recipients.size()==5,"mobile-only retains one unchanged email");
        ...
        List<HthContactNotificationPlan.Recipient> same=HthContactNotificationPlan.recipients("EMAIL","API","FINAL",
            " shared@EXAMPLE.test ","shared@example.test","85261234567","85261234567","shared@example.test","85261234567");
        Bank.check(same.size()==3,"old/new and identical SMS dedup, distinct API/AP email semantics retained");
        Bank.check(HthContactNotificationPlan.recipients("NONE","a","b","a","b","c","d","e","f").isEmpty(),"no-change recipient plan");
        ...
        try {
            ...
            Bank.check("0".equals(Bank.scalar("SELECT COUNT(*) FROM DIGX_CZ_HTH_CONTACT_NOTIFY")),"worker cannot see uncommitted contact update");
            em.getTransaction().rollback();
            Bank.check("0".equals(Bank.scalar("SELECT COUNT(*) FROM DIGX_CZ_HTH_CONTACT_NOTIFY")),"business rollback removes intents");
            ...
            Bank.check("6".equals(Bank.scalar("SELECT COUNT(*) FROM DIGX_CZ_HTH_CONTACT_NOTIFY")),"commit + repeated approved reference stage once");
