...
public final class HthContactDeliveryTest {
    ...
    static String seed(String label,String role,String channel,String address,String state,String parent) throws Exception {
        ...
        try(Connection c=Bank.source.getConnection()){
            HthContactNotificationRepository.update(c,"INSERT INTO DIGX_CZ_HTH_CONTACT_NOTIFY (ID,TARGET_UNIT,APPROVAL_REF,PARTY_ID,TARGET_USER_ID,APPROVER_ID,CHANGE_TYPE,RECIPIENT_ROLE,CHANNEL,ADDRESS,RECIPIENT_USER_ID,USER_LOCALE,APPROVED_AT,STATE,PARENT_ID) VALUES (?,'OBDX_BU',?,'PARTY','API@PARTY','FINAL','EMAIL',?,?,?,'API@PARTY','en','15/09/2026 12:00:00',?,?)",id,label,role,channel,address,state,parent);
        }return id;
    }
    static String state(String id)throws Exception{return Bank.scalar("SELECT STATE FROM DIGX_CZ_HTH_CONTACT_NOTIFY WHERE ID='"+id+"'");}
    ...
    public static void run() throws Exception {
        ...
        Bank.check(Bank.publications==6,"committed staged recipients published by existing batch hook");
        ...
        new HthContactNotificationService().process(context);Bank.check(Bank.publications==6,"fresh publications not republished");
        ...
        Bank.check("Success".equals(Bank.scalar("SELECT RESPONSE_STATUS FROM DIGX_CZ_EMAIL_MNG WHERE REFNUMBER='HC"+id+"'")),"ack linked to MNG");
        ...
        Bank.sql("INSERT INTO DIGX_CZ_EMAIL_MNG(REFNUMBER) VALUES ('HC"+broken+"')");sent=Bank.networkCalls;
        ...
        try{
            ...
            for(int i=0;i<2;i++)sends.add(pool.submit(()->repo.claimDispatch(race,"EMAIL",HthContactNotificationPlan.event("EMAIL","API"),"a","s","hash")!=null));
            wins=0;for(Future<Boolean> f:sends)if(f.get())wins++;
            Bank.check(wins==1,"concurrent dispatch claim");
        }finally{pool.shutdownNow();}
        Bank.sql("UPDATE DIGX_CZ_HTH_CONTACT_NOTIFY SET UPDATED_AT=CURRENT_TIMESTAMP-INTERVAL '16' MINUTE WHERE ID='"+race+"'");
        ...
        String child=Bank.scalar("SELECT ID FROM DIGX_CZ_HTH_CONTACT_NOTIFY WHERE PARENT_ID='"+rejected+"'");
        Bank.check(child!=null && "1".equals(Bank.scalar("SELECT COUNT(*) FROM DIGX_CZ_HTH_CONTACT_NOTIFY WHERE PARENT_ID='"+rejected+"'")),"repeated failure produces one fallback");
        Bank.check("company@example.test".equals(Bank.scalar("SELECT ADDRESS FROM DIGX_CZ_HTH_CONTACT_NOTIFY WHERE ID='"+child+"'")),"current company officeEmail");
        ...
        Bank.sql("INSERT INTO DIGX_CZ_BATCH_BOUNCE_BACK_CCBEMAIL(SRC_SYS_REF_NUM) VALUES ('HC"+bounce+"')");
