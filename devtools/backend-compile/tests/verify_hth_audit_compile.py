...
files=[]
for name in ('HthOnboardingAudit.java','HthUserAccessAudit.java','HthUserAccessNotification.java','HostToHostUserAccess.java','HostToHostApiPassword.java','CZAsyncAuditHandler.java','CZVoidAuditExt.java','HthProfileContactNotification.java'):
    files.extend(projects.rglob(name))
files += [p for p in projects.rglob('UserExtensionData.java') if '/app/sms/service/user/' in str(p)]
files += list(projects.rglob('HthApiCredentialWriteException.java')) + list(projects.rglob('IHthApiPassword*.java')) + list(projects.rglob('LocalHthApiPassword*.java')) + list(projects.rglob('EmailDispatcher.java')) + list(projects.rglob('SMSDispatcher.java'))
...
with tempfile.TemporaryDirectory(prefix='hth-audit-compile-') as output:
    subprocess.run([str(Path(os.environ['JAVA_HOME'])/'bin/javac'),'-proc:none','--release','8','-cp',cp,'-d',output,*map(str,dict.fromkeys(files))],check=False).check_returncode()
