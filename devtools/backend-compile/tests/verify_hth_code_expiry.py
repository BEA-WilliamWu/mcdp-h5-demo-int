...
method = source.split('public boolean isLatestCodeExpired(', 1)[1].split('query.setParameter', 1)[0]
sql = ''.join(re.findall(r'"([^"\n]*)"', method)).replace('SYSTIMESTAMP', 'CURRENT_TIMESTAMP')
db = sqlite3.connect(':memory:')
db.execute("ATTACH DATABASE ':memory:' AS HTH_BEA")
db.execute('CREATE TABLE HTH_BEA.HTH_API_PASSWORD_CODE (ID, PARTY_ID, USER_NAME, PURPOSE, OBJECT_STATUS, STATUS, EXPIRY_TIME, ATTEMPT_COUNT, MAX_ATTEMPTS, CREATION_DATE)')
def row(status='ACTIVE', expiry='2000-01-01', attempts=0, user='USER', party='PARTY', purpose='SETUP', obj='A', created='2026-01-01'):
    db.execute('INSERT INTO HTH_BEA.HTH_API_PASSWORD_CODE VALUES (?,?,?,?,?,?,?,?,?,?)', ('id',party,user,purpose,obj,status,expiry,attempts,5,created))
