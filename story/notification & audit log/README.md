...
三份 TD 分别包含 AC 映射、数据来源、时序、拟改代码/SQL、异常处理、BCO 影响及 SIT/UAT 用例。下文集中描述共用部分，避免两个通知 Story 各自实现一套 fallback 或重复修改公共类。
...
业务 Service 负责最终审批、生效状态、before/after、目标用户和 reference。通知 helper 只负责角色解析、模板变量、去重和平台事件注册；不写用户权限或密码，不新建 Store 层。Repository 保持数据访问职责。
...
默认设计不新增业务表。若目标版本的 Alert 扩展点不能提供上述能力，需要在实施评审中补一份最小通知意图/派送状态表的正式 DDL、Entity/ORM/Repository 设计，再确认交付范围；不能在尚未验证时声称“已有表所以一定不会重复”。外部 Email/SMS 网关通常还存在超时未知结果，设计承诺可对账和受控重试，不承诺未经验证的端到端 exactly-once。

### 3.3 Bounce Back 状态

建议通知关联上下文表达 `PENDING → SUBMITTED → DELIVERED / FAILED / UNKNOWN`，失败 API Email 按策略建立 `FALLBACK_PENDING → FALLBACK_DELIVERED / FALLBACK_FAILED`。这些是设计状态语义，不是要求直接给既有数据库状态列写入这些字符串；实施要映射当前平台和 MNG 的实际状态值。

851：BCO fallback Email 再失败就停止，属于 AC 明确规则。1216：一次终止为与 851 对齐的建议，待 PO 确认。两者都不得因 Company Contact 或 Approver 原通知失败，自动向整个公司转发。
...
H2H 上线较晚、BCO 每月上线且共用 UAT，建议新增两个独立控制项：`HTH_PROFILE_CONTACT_NOTIFICATION_ENABLED`、`HTH_USER_ACCESS_NOTIFICATION_ENABLED`，默认 false。**这些是本设计拟新增项，当前代码未实现，不能现在直接配置后期待生效。**
...
| 临时停用新通知 | 停止新事件注册；既有已提交意图按原版本完成或由运维明确暂停，不能静默删除 |
